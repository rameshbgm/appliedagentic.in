...
// Returns a new MediaAsset record with the cropped file.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Load the original asset record
    ...
    try {
      ...
      return apiError('Source file not found on disk', 404)
    }
    ...
    const { url: newUrl, filename: newFilename, sizeBytes } = await saveFile({
      buffer: outBuffer,
      mimeType: outputMime,
      subDir,
    })

    // Get dimensions of the new file
