...
import { Menu as MenuIcon, X, Search, Zap, ChevronRight, ChevronDown, ArrowRight, Plus, Minus } from 'lucide-react'
              ...
              <div key={menu.id}>
                {hasSubs ? (
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const next = isExpanded ? null : menu.id
                      setOpenMobileMenuId(next)
                      if (next !== null) setOpenMobileSubId(null)
                    }}
                    className={`w-full flex items-center justify-between rounded-xl px-4 py-3 transition-colors ${hoverBg}`}
                    style={{ minHeight: 48, color: isExpanded ? '#3b82f6' : textPrimary, fontSize: '14.5px', fontWeight: 600, fontFamily: FONT, WebkitTapHighlightColor: 'transparent', border: 'none', background: 'transparent', cursor: 'pointer' } as React.CSSProperties}
                  >
                    <span>{menu.title}</span>
                    {isExpanded
                      ? <Minus size={18} style={{ color: '#3b82f6' }} />
                      : <Plus size={18} style={{ color: textMuted }} />}
                  </button>
                ) : (
                  /* Menu without sub-menus: navigate directly */
                  <Link
                    href={`/${menu.slug}`}
                    onClick={() => { closeMobile(); showLoading(`/${menu.slug}`) }}
                    className={`flex items-center rounded-xl px-4 py-3 transition-colors ${hoverBg}`}
                    style={{ minHeight: 48, color: textPrimary, fontSize: '14.5px', fontWeight: 600, fontFamily: FONT }}
                  >
                    {menu.title}
                  </Link>
                )}
                ...
                {hasSubs && isExpanded && (
                  <div className="mt-1 mb-2 space-y-0.5" style={{ borderLeft: '3px solid #3b82f6', marginLeft: 16, paddingLeft: 12 }}>
                    ...
                    <Link
                      href={`/${menu.slug}`}
                      onClick={() => { closeMobile(); showLoading(`/${menu.slug}`) }}
                      className={`inline-flex items-center gap-1 px-3 py-2 rounded-lg text-[11px] font-semibold transition-colors ${hoverBg}`}
                    ...
                    {menu.subMenus!.map((sub) => {
                      ...
                      return (
                        <div key={sub.id}>
                          {/* Sub-menu row: entire row toggles articles, no navigation */}
                          <button
                            type="button"
                            onClick={() => {
                            ...
                            className={`w-full flex items-center justify-between rounded-xl px-3 py-2.5 transition-colors ${hoverBg}`}
                            style={{ minHeight: 44, color: subExpanded ? '#3b82f6' : textPrimary, fontSize: '13.5px', fontWeight: 600, fontFamily: FONT, WebkitTapHighlightColor: 'transparent', border: 'none', background: 'transparent', cursor: 'pointer' } as React.CSSProperties}
                          >
                            <span className="leading-snug text-left">{sub.title}</span>
                            {subExpanded
                              ? <Minus size={15} style={{ color: '#3b82f6' }} />
                              : <Plus size={15} style={{ color: textMuted }} />}
                          </button>
